...
function formatPrice(price) {
    // Display the value after the dot if greater than 0
...
$(document).ready(function () {
    ...
    // Event handler for the promo code input field
    $('[name="wf-form-Promo_code"]').on('input', function () {
        applyPromoCode();
    });

    // Function to apply the promo code
    function applyPromoCode() {
        var promoCodeValue = $('[name="wf-form-Promo_code"]').val().trim().toUpperCase();
    ...
    $('[name="wf-form-Promo_code"]').on('paste', function (e) {
        if (promoCodeApplied) {
            e.preventDefault();
        }
    });
